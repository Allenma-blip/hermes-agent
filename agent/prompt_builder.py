...
def load_lessons_pack() -> str:
    """Load lessons_pack.md from HERMES_HOME and return its content as a
    compact prompt block, or empty string.

    The file is generated by lesson_injector.sh (every 6h) from
    lessons.db.  It contains active behavioral corrections that the
    agent should follow.
    """
    try:
        from hermes_cli.config import ensure_hermes_home
    ...
    lessons_path = get_hermes_home() / "lessons_pack.md"
    ...
    try:
        ...
        if not content:
            return ""
        # Truncate to a reasonable size — 52 lessons ≈ 25 KB, but
        # we cap at CONTEXT_FILE_MAX_CHARS to be safe.
        content = _truncate_content(content, "lessons_pack.md")
        return (
            "# Active Lessons (auto-injected from lessons_pack.md)\n"
            "The following behavioral corrections have been learned from past "
            "interactions.  Follow these rules to avoid repeating past mistakes.\n\n"
            + content
        )
    except Exception:
        return ""
